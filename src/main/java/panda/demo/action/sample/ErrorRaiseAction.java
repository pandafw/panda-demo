...
public class ErrorRaiseAction extends CommonAction {
	...
	public String raise() throws Exception {
		...
		if (Strings.isNotEmpty(appError)) {
			addApplicationError(appError, appError);
		}
		clearApplicationWarnings();
		if (Strings.isNotEmpty(appWarn)) {
			addApplicationWarning(appWarn, appWarn);
		}
		clearApplicationConfirms();
		if (Strings.isNotEmpty(appConf)) {
			addApplicationConfirm(appConf, appConf);
		}
		clearApplicationMessages();
		if (Strings.isNotEmpty(appInfo)) {
			addApplicationMessage(appInfo, appInfo);
		}

		clearSessionErrors();
		if (Strings.isNotEmpty(sesError)) {
			addSessionError(sesError, sesError);
		}
		clearSessionWarnings();
		if (Strings.isNotEmpty(sesWarn)) {
			addSessionWarning(sesWarn, sesWarn);
		}
		clearSessionConfirms();
		if (Strings.isNotEmpty(sesConf)) {
			addSessionConfirm(sesConf, sesConf);
		}
		clearSessionMessages();
		if (Strings.isNotEmpty(sesInfo)) {
			addSessionMessage(sesInfo, sesInfo);
		}

		if (Strings.isNotEmpty(strError)) {
			addActionError(strError);
		}
		if (Strings.isNotEmpty(strWarn)) {
			addActionWarning(strWarn);
		}
		if (Strings.isNotEmpty(strConf)) {
			addActionConfirm(strConf);
		}
		if (Strings.isNotEmpty(strInfo)) {
			addActionMessage(strInfo);
		}
		if (Strings.isNotEmpty(strField)) {
			addFieldError("strField", strField);
		}
		if (Strings.isNotBlank(strExcep)) {
