...
<#if result?has_content>
	...
	<@p.listview id="petcategory_benable"
	...
	<div class="p-tcenter" focusme="true">
		<@p.submit onclick="return petcategory_benable_submit();" icon="icon-benable-execute" label="#(btn-benable-execute)"/>

		<@p.a href="javascript:window.history.back()" btn="default" icon="icon-back" label="#(btn-back)"/>

		<script type="text/javascript"><!--
			function petcategory_benable_submit() {
				return plv_submitCheckedKeys('petcategory_benable');
			}
			
			function onPageLoad() {
				plv_checkAll('petcategory_benable');
			}
		--></script>
	</div>
<#else>
