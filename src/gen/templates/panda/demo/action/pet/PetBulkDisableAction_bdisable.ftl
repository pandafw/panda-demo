...
<#if result?has_content>
	...
	<br/>
	<div class="p-tcenter">
		<@p.submit onclick="return pet_bdisable_submit();" icon="icon-bdisable-execute" label="#(btn-bdisable-execute)"/>

		<@p.a href="javascript:window.history.back()" btn="default" icon="icon-back" label="#(btn-back)"/>

		<script type="text/javascript"><!--
			function pet_bdisable_submit() {
				return plv_submitCheckedKeys('pet_bdisable');
			}
			
			function onPageLoad() {
				plv_checkAll('pet_bdisable');
			}
		--></script>
	</div>
<#else>
